...
TEST_CASE("bambolina_split_00", "[SPLITCHAIN]")
{
  std::tuple<size_t, Geo::Point> pts[] = {
    { 14956468,{ 0.15217908304281988, 0.90754497232924769, -0.042609263748015123 } },
    { 14956466,{ 0.15217907626586863, 0.90754489897518797, -0.042609165355545431 } },
    { 17780342,{ 0.15217898333474522, 0.90754498654513438, -0.042609001844989391 } },
    { 14956469,{ 0.15219005729172796, 0.90767894984637665, -0.042790318549831327 } },
    { 17780346,{ 0.15303741615577032, 0.91795797192452755, -0.056699495759786431 } },
    { 17780347,{ 0.15303750780011799, 0.91795908363092837, -0.056701000078117250 } },
    { 8717067, { 0.15303799509999999,  0.91796499490000005, -0.056708998980000001 } },
    { 17780343,{ 0.15304066544927086, 0.91795950122707515, -0.056710365545731431 } },
    { 8717068, { 0.15801300109999999, 0.90772998329999999, -0.059254985300000000 } },
    { 14956466, { 0.15217907626586863, 0.90754489897518797, -0.042609165355545431 }},
    { 17782354,{ 0.15728017317187554, 0.90923440974692904, -0.058876358160337691 } },
    { 14956468,{ 0.15217908304281988, 0.90754497232924769, -0.042609263748015123 } },
    { 14956469,{ 0.15219005729172796, 0.90767894984637665, -0.042790318549831327 } },
    { 14956469,{ 0.15219005729172796, 0.90767894984637665, -0.042790318549831327 } },
    { 14956468,{ 0.15217908304281988, 0.90754497232924769, -0.042609263748015123 } },
    ...
    { 17780347,{ 0.15303750780011799, 0.91795908363092837, -0.056701000078117250 } },
    { 17780346,{ 0.15303741615577032, 0.91795797192452755, -0.056699495759786431 } },
    { 17782356,{ 0.15304099537395344, 0.91794963946274200, -0.056700241327740819 } },
    { 17780347,{ 0.15303750780011799, 0.91795908363092837, -0.056701000078117250 } },
    { 17780343,{ 0.15304066544927086, 0.91795950122707515, -0.056710365545731431 } },
    { 17782354,{ 0.15728017317187554, 0.90923440974692904, -0.058876358160337691 } },
    { 14956466,{ 0.15217907626586863, 0.90754489897518797, -0.042609165355545431 } },
  ...
  Topo::Wrap<Topo::Type::VERTEX> verts[std::size(pts)];
  std::map<size_t, Topo::Wrap<Topo::Type::VERTEX>> id_to_vert;
  for (const auto& pt : pts)
    id_to_vert[std::get<size_t>(pt)];
  for (auto& vert_el : id_to_vert)
    vert_el.second.make<Topo::EE<Topo::Type::VERTEX>>();
  for (const auto& pt : pts)
    id_to_vert[std::get<size_t>(pt)]->set_geom(std::get<Geo::Point>(pt));
  const int size_ch = 9;
  std::vector<Topo::Wrap<Topo::Type::VERTEX>> vvs(size_ch);
  for(int i = 0; i < size_ch; ++i)
    vvs[i] = id_to_vert[std::get<size_t>(pts[i])];

  auto spl_ch = Topo::ISplitChain::make();
  spl_ch->add_chain(vvs);
  for (int i = size_ch; i < std::size(pts); ++i)
  {
    auto i1 = std::get<size_t>(pts[i]);
    auto i2 = std::get<size_t>(pts[++i]);
    spl_ch->add_connection(id_to_vert[i1], id_to_vert[i2]);
  }
  spl_ch->compute();
  spl_ch->boundaries();
}